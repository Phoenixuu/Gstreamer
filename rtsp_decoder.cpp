...
public:
    ...
    VideoDecoder(const std::string& rtsp_uri) : count(0) {
        ...
        // Set RTSP URI source location
        g_object_set(source, "location", rtsp_uri.c_str(), nullptr);

        // Configure appsink
        g_object_set(sink, "emit-signals", TRUE, "sync", FALSE, nullptr);
        g_signal_connect(sink, "new-sample", G_CALLBACK(on_new_sample), this);

        // Handle pad-added signal for rtspsrc and link to depay
            ...
            if (gst_buffer_map(buffer, &map, GST_MAP_READ)) {
                if (g_strcmp0(format, "I420") == 0) {
                    ...
                    if (map.size >= y_size + 2 * uv_size) {
                        ...
                        cv::waitKey(1);  // Needed to display image in OpenCV window
                    } else {
    ...
    static void on_pad_added(GstElement *src, GstPad *new_pad, GstElement *depay) {
        ...
        if (g_str_has_prefix(new_pad_type, "application/x-rtp")) {
            ...
            if (GST_PAD_LINK_FAILED(ret)) {
                std::cerr << "Failed to link depay to rtspsrc pad\n";
            }
        }
...
//                         // std::cout << decoder->count << std::endl;
//                         cv::imshow("Video Stream", bgr);
//                         cv::waitKey(1); // Add a waitKey to process GUI events
//                     } else {
//                         std::cerr << "Buffer size mismatch for I420 format\n";
//                     }
//                 } else {
